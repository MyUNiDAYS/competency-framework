var Bus = (function () {
	...
	*/
	...
	me.subscribe = function (types, func) {
		var theTypes = typeof types == 'string' ? types.split(',') : types;
		for (var t = 0; t < theTypes.length; t++) {
			var type = theTypes[t];
	...
	me.subscribeOnce = function (type, func) {
		function unsub () {
			me.unsubscribe(func);
			me.unsubscribe(unsub);
		}
		me.subscribe(type, func);
		me.subscribe(type, unsub);
	};
